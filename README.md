...
- Twitter共有: 現在の加工済みプレビュー1枚を共有用 PNG として準備し、画像コピー、Twitter投稿画面オープン、画像保存を分けて実行できます。
...
6. 必要に応じて「Twitterへ共有」から現在の加工済みプレビュー1枚を共有します。
7. 「一括保存」で ZIP をダウンロードします。

## プライバシー

画像処理はブラウザの File API と Canvas で行います。画像のアップロード、サーバー処理、テレメトリ送信はありません。Twitter共有は現在の加工済み画像1枚をブラウザ内で生成し、ユーザー操作でクリップボードコピー、Twitter投稿画面オープン、または画像保存を行います。投稿や画像貼り付けはユーザー操作に委ねられます。
